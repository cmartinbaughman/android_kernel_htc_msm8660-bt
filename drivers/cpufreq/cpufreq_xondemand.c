...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_XONDEMAND
static
#endif
struct cpufreq_governor cpufreq_gov_xondemand = {
       .name                   = "xondemand",
...
static void xondemand_powersave_bias_init_cpu(int cpu)
{
...
static void xondemand_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		xondemand_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_xondemand Governor Tunables */
...
#ifndef CONFIG_CPU_FREQ_GOV_XONDEMAND_2_PHASE
...
static void dbs_chown(void)
{
	...
	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ignore_nice_load", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown ignore_nice_load returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/io_is_busy", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown io_is_busy returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/powersave_bias", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown powersave_bias returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/sampling_down_factor", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown sampling_down_factor returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown sampling_rate returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/two_phase_freq", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown two_phase_freq returns: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/two_phase_dynamic", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown two_phase_dynamic returns: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/two_phase_bottom_freq", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown two_phase_bottom_freq returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/up_threshold", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown up_threshold returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/down_differential", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown down_differential returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/touch_poke", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown touch_poke returns: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/input_boost_duration", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown input_boost_duration returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ui_sampling_rate", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown ui_sampling_rate returns: %d", ret);

	ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ui_counter", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_warn("sys_chown ui_counter returns: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ux_freq", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ux_freq error: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ux_loading", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ux_loading error: %d", ret);

    ret = sys_chown("/sys/devices/system/cpu/cpufreq/xondemand/ux_boost_threshold", low2highuid(AID_SYSTEM), low2highgid(0));
	if (ret)
		pr_err("sys_chown ux_boost_threshold error: %d", ret);
}

static void dbs_refresh_callback_xondemand(struct work_struct *unused)
{
	struct cpufreq_policy *policy;
...
static DECLARE_WORK(dbs_refresh_work, dbs_refresh_callback_xondemand);
...
static int cpufreq_xondemand_input_boost_task (
   void *data
   )
{
    struct cpufreq_policy *policy;
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
        ...
        if (!cpu) {
            if (!boost_task_alive) {
                input_boost_task = kthread_create (
                   cpufreq_xondemand_input_boost_task,
                   NULL,
                   "kinputboostd"
                   );
		...
		this_dbs_info->rate_mult = 1;
		xondemand_powersave_bias_init_cpu(cpu);
		/*
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_xondemand);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_xondemand);
}


MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_xondemand' - A dynamic cpufreq governor for "
